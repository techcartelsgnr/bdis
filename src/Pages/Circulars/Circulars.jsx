...
import { getCircularsData } from '../../Redux/UserSlice';
...
const Circulars = () => {
  ...
  const fetchCircularsData = async () => {
            const res = await dispatch(getCircularsData()).unwrap()
            setCircularsData(res.circulars);
            console.log("Circulars Data", res.circulars);
       }
       useEffect(() => {
            fetchCircularsData()
       }, [])
  ...
  if (!circularsdata) return <p>No data found</p>;
          ...
          <main className="main">
            ...
            <div
              ...
              <div className="container">
                <h2 className="breadcrumb-title">Circulars</h2>
                <ul className="breadcrumb-menu">
                  <li>
                    <Link to="/" style={{color: 'yellow'}}>Home</Link>
                  </li>
                  <li className="active" style={{color: 'yellow'}}>Circulars</li>
                </ul>
              </div>
            </div>
                  ...
                  <div className="row">
                    <div className="col-xl-12 col-lg-12">
                      <div className="department-details">
                        <h3 className="mb-20">Circulars</h3>

                        {/* <p className="mb-20">
                          {mandatorydata[0].desp}
                        </p> */}
                        <SummernoteContent content={circularsdata && circularsdata[0]?.desp} />
                        <div className="table-responsive mt-4">
                          <table className="table table-bordered">
                            ...
                            <tbody>
                              {circularsdata && [...circularsdata].reverse().map((item, index) => (
                                <tr key={index}>
                                  <td>{index + 1}</td>
                                  <td>{item.title}</td>
                                  <td>
